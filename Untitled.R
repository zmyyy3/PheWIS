phewis <-
  function(phenotypes,genotypes,exposers,data,covariates=c(NA),adjustments=list(NA), outcomes, predictors, cores=1, additive.genotypes=T, 
           significance.threshold, alpha=0.05, unadjusted=F, return.models=F, min.records=20, MASS.confint.level=NA,quick.confint.level,
           clean.phecode.predictors=F) { ## MZ
    ...
    if(missing(data)) {
      ...
      id=intersect(intersect(names(phe),names(gen)),names(exp)) ##MZ
      if(length(id)==0) {stop("There is no shared column to merge phenotypes, genotypes and exposers!")} ##MZ
      ...
      exposers=names(exposers) ##MZ
      exposers=exposers[!(exposers %in% id)] ##MZ
      if(length(phenotypes)<1 || length(genotypes)<1) 
      {stop("Either phenotypes, genotypes or exposers contained no non-shared columns, yielding no variables for analysis after the merge.")} ##MZ
      data=merge(merge(phe,gen,by=id),exp,by=id) ##MZ
    ...
    full_list=data.frame(t(expand.grid(phenotypes,genotypes,exposers,covariates,adjustments,stringsAsFactors=F)),stringsAsFactors=F)

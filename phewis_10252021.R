phewis <-
  function(phenotypes,genotypes,exposures,data,covariates=c(NA),adjustments=list(NA), outcomes, predictors, cores=1, additive.genotypes=T, 
    ...
    if(missing(data)) {
      ...
      if(length(id)==0) {stop("There is no shared column to merge phenotypes, genotypes and exposure!")} ##MZ
      ...
      exposures=names(exposures) ##MZ
      exposures=exposures[!(exposures %in% id)] ##MZ
      if(length(phenotypes)<1 || length(genotypes)<1) 
      {stop("Either phenotypes, genotypes or exposure contained no non-shared columns, yielding no variables for analysis after the merge.")} ##MZ
    ...
    full_list=data.frame(t(expand.grid(phenotypes,genotypes,exposures,adjustments,stringsAsFactors=F)),stringsAsFactors=F)

phe_as <-
  function(phe.gen, additive.genotypes=T,min.records=20,return.models=F,confint.level=NA, my.data, my.covariates) {
    ...
    }else{ model_name=sprintf("No model: %s ~ %s",phe,paste(paste0(names(d)[c(1,3)],collapse = " * "),"+",paste0(names(d)[4:length(names(d))],collapse = "+")))
    }  ## MZ
    }
    if(n_total< min.records) {
      ...
      if(class(d[[phe]]) %in% c("logical")) {
        ...
        if(n_cases<min.records|n_controls<min.records) {note=paste(note,"[Error: <",min.records," cases or controls]")}else{ 
          ...
          }else{ model=glm(as.formula(sprintf("%s ~ %s",phe,paste(paste0(names(d)[c(1,3)],collapse = " * "),"+",paste0(names(d)[4:length(names(d))],collapse = "+")))), data=d, family=binomial)
          }  ## MZ
          }  ## MZ
          ## MZ
    ...
    if (length(covariates) > 1) { covariates=paste0(names(d)[4:length(names(d))],collapse="+") }
    output=data.frame(phenotype=phe_o,snp=gens,exposure=exp,
